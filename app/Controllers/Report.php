...
class Report extends BaseController
{
    ...
    public function sop()
    {
        ...
        if ($this->data['outletPick'] === null) {
            $sopdetails = $SopDetailModel->orderby('created_at', 'DESC')->where('created_at >=', $startdate . ' 00:00:00')->where('created_at <=', $enddate . ' 23:59:59')->find();
            $addres = "All Outlets";
            $outletname = "58vapehouse";
        } else {
            $sopdetails = $SopDetailModel->orderby('created_at', 'DESC')->where('outletid', $this->data['outletPick'])->where('created_at >=', $startdate . ' 00:00:00')->where('created_at <=', $enddate . ' 23:59:59')->find();
            $outlets = $OutletModel->find($this->data['outletPick']);
            $addres = $outlets['address'];
            $outletname = $outlets['name'];
        }
